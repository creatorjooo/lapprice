...
export default function Footer({ onNavigateToPage }: FooterProps) {
  const footerLinks = {
    laptop: {
      title: '노트북',
      links: [
        { label: 'Apple', action: () => onNavigateToPage?.('laptop-apple') },
        { label: '게이밍', action: () => onNavigateToPage?.('laptop-gaming') },
        { label: '울트라북', action: () => onNavigateToPage?.('laptop-ultrabook') },
        { label: '비즈니스', action: () => onNavigateToPage?.('laptop-business') },
        { label: '크리에이터', action: () => onNavigateToPage?.('laptop-creator') },
        { label: '가성비', action: () => onNavigateToPage?.('laptop-budget') },
      ],
    },
    monitor: {
      title: '모니터',
      links: [
        { label: '게이밍 모니터', action: () => onNavigateToPage?.('monitor-mon-gaming') },
        { label: '전문가용', action: () => onNavigateToPage?.('monitor-mon-pro') },
        { label: '울트라와이드', action: () => onNavigateToPage?.('monitor-mon-ultrawide') },
        { label: '가성비', action: () => onNavigateToPage?.('monitor-mon-general') },
      ],
    },
    desktop: {
      title: '데스크탑',
